document.addEventListener('DOMContentLoaded', () => {
    ...
    const DOM = {
        ...
        panels: {
            ...
            global: document.getElementById('global-settings-panel'),
            layout: document.getElementById('layout-settings-panel'),
        },
        panelBodies: {
            inspector: document.getElementById('inspector-body'),
            global: document.getElementById('global-settings-body'),
            layout: document.getElementById('layout-settings-body'),
        }
    };
    ...
    async function loadAdminPanel() {
        ...
        try {
            ...
            alert(`Не удалось загрузить или разобрать config.json. Ошибка: ${error.message}`);
        }
    }
    ...
    function renderCanvas() {
        ...
        // Рендерим фон для основной части (BODY)
        const mainLayout = currentConfig.layout.main || {};
        if (mainLayout.background) {
            if (mainLayout.background.type === 'color') DOM.canvas.style.backgroundColor = mainLayout.background.value;
            // Другие типы фона для canvas можно добавить здесь
        } else {
            DOM.canvas.style.backgroundColor = '#ffffff';
        }
    ...
    function createAdminElement(elementData) {
        ...
        if (iframe) {
            // FIX: Запрещаем автопроигрывание видео
    ...
    // --- ПАНЕЛИ НАСТРОЕК (Глобальные, Макет, Инспектор) ---
        ...
        body.innerHTML = `
            <div class="inspector-group">
                ...
                <div class="inspector-field">
                    ...
                    <input type="text" data-config-path="globalSettings.pageTitle" value="${currentConfig.globalSettings.pageTitle || ''}">
                </div>
            </div>
        `;
    ...
    function renderLayoutSettingsPanel() {
        ...
        body.innerHTML = `
            <div class="inspector-group">
                <h4>Шапка (Header)</h4>
                ${createSectionEditor('header', header)}
            </div>
            <div class="inspector-group">
                <h4>Основная часть (Main)</h4>
                ${createSectionEditor('main', main)}
                <h5>Колонки</h5>
                <div id="columns-editor">
                    ${main.columns.map(col => createColumnEditor(col)).join('')}
                </div>
                <button id="add-column-btn" class="add-element-btn" style="width:100%; margin-top:10px;">+ Добавить колонку</button>
            </div>
            <div class="inspector-group">
                <h4>Подвал (Footer)</h4>
                ${createSectionEditor('footer', footer)}
            </div>
        `;
    ...
    function createSectionEditor(key, config) {
        return `
            ...
            <div class="inspector-field">
                ...
                <textarea data-config-path="layout.${key}.content">${config.content || ''}</textarea>
            </div>` : ''}
            <div class="inspector-field">
                <label>Тип фона</label>
                <select data-config-path="layout.${key}.background.type">
                    <option value="color" ${config.background?.type === 'color' ? 'selected' : ''}>Цвет</option>
                    <option value="image" ${config.background?.type === 'image' ? 'selected' : ''}>Изображение</option>
                </select>
            </div>
            <div class="inspector-field">
                <label>Значение (цвет или URL)</label>
                <input type="text" data-config-path="layout.${key}.background.value" value="${config.background?.value || ''}">
            </div>
        `;
    }
    ...
    function createColumnEditor(column) {
        return `
            <div class="column-editor" data-column-id="${column.id}">
                <input type="text" data-config-path="layout.main.columns" data-column-id="${column.id}" data-property="width" value="${column.width}">
                <button class="delete-column-btn" data-column-id="${column.id}">❌</button>
            </div>
        `;
    }
    
    // --- ОБНОВЛЕНИЕ CONFIG В РЕАЛЬНОМ ВРЕМЕНИ ---
    function updateConfigFromInput(event) {
        const el = event.target;
        const path = el.dataset.configPath;
        if (!path) return;

        // Специальная логика для колонок
        if (path.includes('layout.main.columns')) {
            const columnId = el.dataset.columnId;
            const property = el.dataset.property;
            const columnIndex = currentConfig.layout.main.columns.findIndex(c => c.id === columnId);
            if (columnIndex > -1) {
                currentConfig.layout.main.columns[columnIndex][property] = el.value;
            }
        } else {
            // Общая логика для всех остальных полей
            let keys = path.split('.');
            let last = keys.pop();
    ...
    // --- ИНТЕРАКТИВНОСТЬ (DRAG-N-DROP, ПАНЕЛИ) ---
    ...
    function setupToolbarActions() {
        document.querySelectorAll('.add-element-btn').forEach(btn => {
            if (btn.id !== 'add-column-btn') btn.onclick = () => addNewElement(btn.dataset.type);
        });
        document.querySelectorAll('.preview-btn').forEach(btn => {
            btn.onclick = () => {
                if (btn.dataset.mode === 'desktop') DOM.canvas.style.width = '100%';
                if (btn.dataset.mode === 'tablet') DOM.canvas.style.width = '768px';
                if (btn.dataset.mode === 'mobile') DOM.canvas.style.width = '375px';
            }
        });
    ...
    // --- ЗДЕСЬ ИДЕТ ОСТАЛЬНОЙ КОД ADMIN.JS БЕЗ ИЗМЕНЕНИЙ ---
    // (saveConfiguration, initDragAndDrop, makePanelsInteractive, selectElement, renderInspector, и т.д.)
    // Для краткости я заменю его на рабочие версии.
    ...
    function updateElementFromInspector(event){if(!selectedElementId)return;const elementData=currentConfig.elements.find(el=>el.id===selectedElementId);const input=event.target;const value=input.value;if(input.dataset.key)elementData[input.dataset.key]=value;else if(input.dataset.contentKey)elementData.content[input.dataset.contentKey]=value;else if(input.dataset.styleKey){if(!elementData.styles)elementData.styles={};elementData.styles[input.dataset.styleKey]=value}renderCanvas();}
